...
public class IndividualKeyScript : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        //if (AI)
        //{
        //    Collider[] hitColliders = Physics.OverlapSphere(keyPos, 0.1f);
        ...
        else if (!holdingNote && currentNote != null)
        {
            StopNote();
        }
    ...
    public void PlayNote(bool wasAI)
    {
        ...
        heldNotedistance = 0;
        if (Physics.Raycast(keyPos, transform.up, out hit, 0.5f))//Shoots raycast from the tip of note
        {
    ...
    void OnDrawGizmos()
    {
        ...
        Gizmos.DrawSphere(new Vector3(keyPos.x, keyPos.y + 0.5f, keyPos.z), 0.1f);

        Gizmos.DrawLine(keyPos, new Vector3(keyPos.x, keyPos.y + 0.5f, keyPos.z));
    }

    void OnTriggerEnter(Collider collider)
    {
        if (collider.tag == "NoteZone" && AI)
        {
            PlayNote(AI);
        }
    }
