...
public class IndividualKeyScript : MonoBehaviour
{
    ...
    bool isMultiplayer;
    public List<NoteClass> playerRecord;
    ...
    void Start()
    {
        ...
        isMultiplayer = true;
        playerRecord = new List<NoteClass>();
	}
    ...
    public void Update()
    {
		...
		if(AI)
		{
			if (player3or4)
			{
				if (whiteKey)
				{
					if (Physics.Raycast(keyPos + Vector3.down * 2.5f, Vector3.down, out hit, .5f) && hit.collider.gameObject.tag == "Note")//Shoots raycast from the tip of note
					{
						StopNote();
						PlayNote();
						AITimer = 0;
		...
		if (holdingNote)//If the holding note has been hit
		{
			if(player3or4)
			{
				if (whiteKey)
				{
					if (Physics.Raycast(keyPos + Vector3.down * 2.5f, Vector3.down, out hit, heldNotedistance + 1f))//Shoots raycast from the tip of note
					{
				...
				{
					if (Physics.Raycast(keyPos + Vector3.down * 1.5f, Vector3.down, out hit, heldNotedistance + 1f))//Shoots raycast from the tip of note
					{
						if (hit.collider.tag == "HeldNote")//If the raycast hits a note that must be held...
						{
							//Debug.Log("HELD!!!!!!");
    ...
    public void PlayNote()
	{
		...
		if(player3or4)
		{
			...
			if (whiteKey)
			{
				...
				if (Physics.Raycast(keyPos + Vector3.down * 2.5f, Vector3.down, out hit, 1))//Shoots raycast from the tip of note
				{
			...
			{
				if (Physics.Raycast(keyPos + Vector3.down * 1.5f, Vector3.down, out hit, 1))//Shoots raycast from the tip of note
				{
					if (hit.collider.tag == "Note" || hit.collider.tag == "SharpNote")//If the raycast hits a regular or sharp note...
					{
						if (hit.distance <= .7f && hit.distance > .45f)//Sweet spot distance
