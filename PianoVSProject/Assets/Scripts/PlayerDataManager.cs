    ...
    public void SetAI(bool on)//Set AI as on or off
    {
        if (on && isAI == false)
        {
            ...
            foreach (IndividualKeyScript keyScript in transform.GetComponentsInChildren<IndividualKeyScript>())
            {
                keyScript.AI = true;
                keyScript.StopNote();
                keyScript.gameObject.GetComponent<AudioSource>().volume = 0.8f;
				keyScript.startingVolume = 0.8f;
			}
        }
        else if (!on && isAI == true)
        {
            ...
            foreach (IndividualKeyScript keyScript in transform.GetComponentsInChildren<IndividualKeyScript>())
            {
                keyScript.AI = false;
                keyScript.StopNote();
                keyScript.gameObject.GetComponent<AudioSource>().volume = 1;
				keyScript.startingVolume = 1;
			}
        }
    }
